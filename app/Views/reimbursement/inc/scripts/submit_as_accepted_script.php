<script>
    {
        const formReimAsAccepted = $("#as_accepted_reimbursement_form");
        formReimAsAccepted.submit((e) => {
            e.preventDefault();
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setOnProgress(true);
                    let data = new FormData(formReimAsAccepted[0]);
                    $.ajax({
                        type: "post",
                        url: formReimAsAccepted.attr('action'),
                        data: data,
