<script>
    ...
    {
        const formDraftReimbursement = $("#draft_reimbursement_form");
        formDraftReimbursement.submit((e) => {
            e.preventDefault();
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setOnProgress(true);
                    let data = new FormData(formDraftReimbursement[0]);
                    $.ajax({
                        type: "post",
                        url: formDraftReimbursement.attr('action'),
                        data: data,
