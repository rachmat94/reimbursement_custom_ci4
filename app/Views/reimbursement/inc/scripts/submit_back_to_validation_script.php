<script>
    {
        const formBackToValidationReim = $("#back_to_validation_reim_form");
        formBackToValidationReim.submit((e) => {
            e.preventDefault();
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    setOnProgress(true);
                    let data = new FormData(formBackToValidationReim[0]);
                    $.ajax({
                        type: "post",
                        url: formBackToValidationReim.attr('action'),
                        data: data,
