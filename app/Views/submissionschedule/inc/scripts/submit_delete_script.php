<script>
    {
        const formDeleteSubmissionSchedule = $("#delete_submissionschedule_form");
        formDeleteSubmissionSchedule.submit((e) => {
            e.preventDefault();
            Swal.fire({
                title: 'Are you Sure?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    setOnProgress(true);
                    let data = new FormData(formDeleteSubmissionSchedule[0]);
                    $.ajax({
                        type: "post",
                        url: formDeleteSubmissionSchedule.attr('action'),
                        data: data,
                        ...
                        success: function(response) {
                            ...
                            if (response.code == "success") {
                                $("#delete_submissionschedule_modal").modal("hide");
                                setTimeout(() => {
