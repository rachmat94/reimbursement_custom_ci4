<script>
    {
        const formDeleteJenisBerkas = $("#delete_jenisberkas_form");
        formDeleteJenisBerkas.submit((e) => {
            e.preventDefault();
            Swal.fire({
                title: 'Are you Sure?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    setOnProgress(true);
                    let data = new FormData(formDeleteJenisBerkas[0]);
                    $.ajax({
                        type: "post",
                        url: formDeleteJenisBerkas.attr('action'),
                        data: data,
                        ...
                        success: function(response) {
                            ...
                            if (response.code == "success") {
                                $("#delete_jenisberkas_modal").modal("hide");
                                setTimeout(() => {
